...
PARTS_JS = "IndustryExplorer.semi(P);"
...
def parts_html(d):
    ...
    embed = {
        ...
        "pages": sorted({c["stock"] for rows in d["cos"].values() for c in rows}
                        | {c["stock"] for per in d["partcos"].values() for rows in per.values() for c in rows}
                        | {c["stock"] for c in d["unspec"]}),
    }

    embed["pages"] = [stock for stock in embed["pages"]
                      if os.path.isfile(os.path.join(KSEMI, stock, "index.html"))]
    ...
    body = """
<div class="kpi">%s</div>
<section class="card"><h2>팹 공정 흐름 <em>공정 → 하위 스텝·구성 부품 → 연결 기업</em>
...
""" % ("".join(kpi), svg(d), legend, len(d["untagged"]), untag_rows,
    ...
    return page("팹 공정 흐름 인포그래픽 — 단계 · 장비사 · 부품사", body, depth=0,
                ...
                lead="웨이퍼 투입에서 패키징까지 팹 공정을 단계로 나누고, 각 단계의 장비를 만드는 국내 상장사와 "
                     "그 장비의 부품·부품사를 원문 근거와 함께 연결했습니다. 공정을 선택하고, 하위 스텝이나 부품을 눌러 연결 기업을 확인하세요.")
