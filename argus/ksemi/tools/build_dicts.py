...
# CMP 소모품 낱말은 **부품(cmp.pad) 전용**이고 단계 낱말이 아니다 — 패드를 만드는 회사는
# CMP 장비사가 아니라 부품소재사다. 단계 strong/weak 에 넣으면 소모품사가 CMP 장비사로
# 태깅된다. 같은 이유를 여덟 낱말에 똑같이 단다.
for _w in ("cmp 패드", "cmp패드", "연마패드", "연마 패드", "polishing pad",
           "리테이너링", "리테이너 링", "retainer ring"):
    UNASSIGNED_NOTE[_w] = (
        "cmp 단계의 부품 `pad`(CMP 패드·리테이너링) 전용 낱말이다. 패드를 만드는 회사는 "
        "CMP 장비사가 아니라 부품소재사이므로 단계 낱말로는 쓰지 않는다. "
        "이 낱말들은 CMP 패드 낱말이 한때 `링` 하나여서 식각 소모품인 포커스링 업체"
        "(하나머티리얼즈·비씨엔씨·월덱스)가 CMP 패드로 걸린 것을 고치며 들어왔다.")
del _w
...
STAGES = [
    ...
    ("cmp", "CMP", 6, "전공정",
     ...
     [("pad", "CMP 패드·리테이너링",
       ["cmp 패드", "cmp패드", "연마패드", "연마 패드", "polishing pad",
        "리테이너링", "리테이너 링", "retainer ring"]),
      ("chamber", "챔버", ["챔버"]),
      ("jig", "지그·치구", ["지그"])],
     ...
     "CMP 패드 낱말이 한때 `링` 하나였는데 그러면 **식각 소모품인 포커스링** 업체"
     "(하나머티리얼즈·비씨엔씨·월덱스)가 CMP 패드로 걸렸다 — 포커스링은 etch 의 "
     "`focusring` 이지 CMP 패드가 아니다. 그래서 CMP 패드는 `cmp 패드`·`연마패드`·"
     "`리테이너링` 처럼 **부품 이름을 다 적은 낱말로만** 잡는다."),
