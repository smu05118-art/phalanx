...
# 정밀 경로(CORP)에 올라간 상장사는 이미 수록 중이므로 커버리지 지도에도 반드시 떠야
# 한다 — 업종 필터에 걸리지 않는다고 지도에서 빠지면 "수록했는데 안 보이는" 상태가 된다.
SEED_EXTRA = {
    "028260": "삼성물산 — 업종은 기타 전문 도매업이나 건설부문이 주력 (원본 7사)",
    "028050": "삼성E&A — 업종은 건축기술 엔지니어링이나 EPC 시공사 (원본 7사)",
    "317400": "자이에스앤디 — 업종 밖이나 GS건설 계열 시공사로 정밀 경로 수록 중",
}

# 정밀 경로 슬러그. `stock=None`은 비상장 자회사(현대엔지니어링·자이씨앤에이·DL건설)로
# 상장사 모집단의 원소가 아니다 — 모회사 보고서를 통해 들어오므로 여기서는 제외한다.
LEGACY_SLUG = {v["stock"]: co for co, v in CORP.items() if v.get("stock")}
