...
_PAREN_NOTE = re.compile(r"\((?:주\s*\d+|%|\*+\d*)\)")   # (주1)·(%)·(*)·(*1) 주석 접미
...
COL_ALIAS = {
    ...
    "완공예정일": "ed", "종료예정일": "ed", "준공예정일": "ed", "계약상완성기한": "ed", "완성기한": "ed",
    "기본도급액": "amt", "기본도급금액": "amt", "수주총액": "amt", "도급액": "amt",
    "완성공사액": "cmp", "기납품액": "cmp",
    "계약잔액": "bal", "수주잔고": "bal",
    "진행률": "pr",
    # ── 표준 제조업형 수주상황표(기업공시서식 II-4 표준 양식) ─────────────
    # `품목|수주일자|납기|수주총액(수량,금액)|기납품액(수량,금액)|수주잔고(수량,금액)`.
    # 다층 헤더 평탄화 후의 이름이라 '수주총액'과 키가 다르다. **수량 열은 일부러
    # 매핑하지 않는다** — 금액이 아니라 개수여서 합산하면 집계가 망가진다.
    "수주총액금액": "amt", "기납품액금액": "cmp", "수주잔고금액": "bal",
    "계약금액금액": "amt", "매출액금액": "cmp",          # 동신건설 변형
    "납기": "ed", "납기(완공예정일)": "ed",
    "품목(계약명)": "nm", "수주일자(계약일자)": "sd",
    # ── 건설 현장형 방언(중견 건설사) ────────────────────────────────
    # 태영건설 `수주내용`·계룡건설 `총도급금액`·동부건설 `도급액(당분기)` 등.
    "수주내용": "nm",
    "도급금액": "amt", "총도급금액": "amt",
    "도급액(당분기)": "amt", "도급액(당반기)": "amt", "도급액(당기)": "amt",
    "착공일": "sd", "계약일자": "sd",
    "준공일": "ed", "완공일자": "ed", "계약종료": "ed",
    "누적기성고": "cmp", "완공공사액": "cmp", "누계기성금액": "cmp",
    "차기이월": "bal", "잔여기성금액": "bal",
    # III-8 (진행률 적용 수주계약)
    "회사명": "p8_ent",
