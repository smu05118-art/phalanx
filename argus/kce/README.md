...
신규 편입사 29사에도 현장 선택 → 분기별 공시 대조 → 원문 확인을 제공한다.
`tools/kce_detail.py`가 기존 II-4 원장과 같은 접수번호의 III-8 진행률 적용 수주계약과
건설·공사계약 주석을 추가로 확인한다. `tools/assets/detail_cache/`에 출처 URL,
본문 해시, 표·행, 원문 단위와 연결/별도 표기를 보존한다. 전체 원문 보관 위치는
수집 시 `--raw-dir`로 지정하며 공개 산출물에 포함하지 않는다.

현장 상세에는 잔고·누적 완성공사액, 공시 진행률, 미청구공사·계약자산, 공사미수금,
손상누계·대손충당금·계약부채와 일정·계약금액 변경을 표시한다. II-4 누계 차분은
계산값이며 회사 매출액으로 표시하지 않는다. 없는 수주총액·잔고·예측값을 만들지 않는다.

진행률 표에 수주총액 열이 없어도 실제 공시된 항목을 읽는다. 연결/별도는 표의
명시 문맥으로만 분류하고, 단위가 불명확하면 금액을 비운다. 이름과 계약 식별 근거가
부족한 행은 별도 공시 탐색에 남기며 기존 현장 잔고 합계에 더하지 않는다.
원문의 회사명 열이 원청사·발주처·현장명을 뜻하는 경우도 있으므로 법인 귀속으로 단정하지 않는다.

2026-09-14 보강: 226개 분기 보고서의 349개 절 확인. 28사에서 5,148개 계약 관측을
추출해 3,269개 관측을 기존 595개 현장에 연결했다. 연결/별도는 별개 관측이며,
중요 계약 일부를 담은 표의 합을 회사 전체 값으로 쓰지 않는다. KC산업은 기존 연간
수주 원장을 유지하며 인식된 추가 진행률 표가 없다.
...
python3 kce_detail.py                       # III-8·계약 주석 추가 수집(동일 접수번호 no-op)
