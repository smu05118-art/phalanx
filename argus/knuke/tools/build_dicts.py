...
DOMAINS = [
    ("NUKE", "원자력", "Nuclear", 1,
     ["신한울", "신고리", "새울", "한빛", "한울", "월성", "고리", "울진", "영광",
      "두코바니", "Dukovany", "테멜린", "Temelin", "바라카", "Barakah", "BNPP", "체코원전",
     ...
     ["원자력", "원전", "원자로", "핵연료", "방사선", "사용후핵연료", "계속운전",
      ...
      "중대사고", "MACST", "방폐물",
      # 수출 계약은 계약명이 영문이다(실측: 두산에너빌리티·KC코트렐·한전기술 공시).
      "Nuclear"]),
    ("THERMAL", "화력", "Thermal", 2,
     ["삼척화력", "신서천", "강릉안인", "고성하이", "당진", "태안", "보령", "하동",
      "여수화력", "영흥", "신인천", "안동복합", "통영복합", "신호남", "신일산"],
     ["화력", "석탄", "무연탄", "유연탄", "복합화력", "복합발전", "LNG복합", "가스복합",
      "석탄화력", "열병합", "집단에너지", "화력발전", "석탄가스화", "IGCC",
      # 영문 계약명 — `Thermal Power Station/Project`·`TPP`·`CCGT`·`CCPP`·`Cogeneration`
      # 은 실제 공시 계약명에서 관찰된 표기다. 공백은 매칭 전에 지워지므로 붙여 쓴 것과 같다.
      "Thermal Power", "TPP", "TPS", "CCGT", "CCPP", "Combined Cycle", "Cogeneration",
      "Coal Fired", "Coal-Fired", "보일러발전", "천연가스발전", "파워블록", "가스터빈",
      "디젤파워플랜트", "엔진발전",
      # 보일러를 때는 발전은 화력이다 — 산업용보일러(IND)가 더 길어 먼저 걸린다.
      "보일러",
      # 회처리설비(비회·저회)는 석탄화력 전용 설비다(실측: KC코트렐 Jawa #9&10 공급계약).
      "회처리설비", "Ash Handling",
      # 디젤·가스 엔진 발전소(인도네시아 PLN 패키지) — 연소 발전이므로 화력으로 묶는다.
      "디젤발전", "Diesel Power"]),
    ("RENEW", "신재생", "Renewables", 3,
     ["해상풍력", "새만금", "탐라해상", "서남해", "제주한림", "안마"],
     ["풍력", "태양광", "수소", "연료전지", "신재생", "재생에너지", "그린수소",
      "암모니아", "풍력발전", "조력", "지열", "바이오매스", "ESS", "수전해", "수력",
      "Hydroelectric", "Hydro Power", "Offshore Wind", "Wind Farm", "Photovoltaic",
      "Solar Power", "Fuel Cell"]),
    ("GRID", "송변전", "Transmission", 4,
     ["동해안변환소", "북당진"],
     ["송변전", "변전소", "송전", "배전", "변압기", "개폐기", "가스절연", "HVDC",
      "전력계통", "계통연계", "송전선로", "초고압", "수배전", "Substation"]),
    # 발전기자재사가 같은 양식으로 올리는 **기타 산업설비**(제철·석유화학·담수) — 버리지 않고 갈라 둔다.
    ("IND", "기타 산업설비", "Industrial plant", 5,
     ["사우디", "쿠웨이트", "UAE"],
     ["제철설비", "석유화학", "정유", "담수", "해수담수", "담수화", "환경설비",
      "산업용보일러", "산업설비", "폐열회수", "제강", "소각", "소결", "배가스",
      "화공플랜트", "LNG플랜트", "양극재",
      "Desalination", "Refinery", "Petrochemical"]),
]
...
CONTRACT_TYPES = [
    ...
    ("INSP", "검사·방사선관리", "Inspection & RP", 8,
     ...
     r"안전성\s*평가|검사\s*용역|계측검교정|방사능\s*측정|장비\s*교정|검교정", None),
    ("OM", "정비·O&M", "Maintenance / O&M", 6,
     r"경상정비|예방정비|계획예방정비|정비공사|정비\s*용역|성능\s*개선|개보수|O\s*&\s*M|"
     r"유지보수|오버홀|점검|정비\b|설비진단|화력정비|발전설비정비|"
     # 한전산업의 '연료·환경설비 위탁운전용역'이 이 갈래다(실측 공시 계약명).
     r"위탁운전|운전위탁|운전\s*용역|운영\s*용역|운영위탁|관리운영|유지관리|Maintenance", None),
    ("ENG", "설계·엔지니어링", "Design & engineering", 4,
     r"종합설계|기본설계|상세설계|설계\s*용역|엔지니어링|기술\s*용역|감리|사업관리|"
     r"설계\b|형상관리|타당성|기술지원|인허가|Engineering\b", None),
    # 설계·정비 다음, 기기 갈래보다 앞 — `…발전사업 건설공사`·`EPC 사업`은 기기 공급이 아니라
    # 설치·시공 계약이다(실측: 한전기술 EPC 공사, 두산 공사수주 구분).
    ("EPC", "EPC·건설·설치", "EPC & construction", 3,
     r"EPC|턴키|Turn\s*-?\s*key|건설공사|신축공사|설치공사|토목공사|시공\s*계약|"
     r"플랜트\s*공사|파워블록\s*설치|구축공사|조성\s*사업|주설비공사|전기공사|"
     r"제조위탁설치|위탁설치계약", None),
    ("INC", "계측제어", "I&C", 7,
     r"계측제어|계측기|제어시스템|I\s*&\s*C|MMIS|RSP|원전계측|디지털제어|"
     r"감시제어|안전등급\s*제어|원자로보호계통|ICI\b|RTD\b|In-?core|노내계측|계측케이블", None),
    ("MAIN", "주기기", "NSSS / main equipment", 1,
     r"원자로설비|원자로\b|증기발생기|가압기|냉각재펌프|주기기|핵증기공급|NSSS|"
     r"증기터빈|가스터빈|터빈발전기|터빈|발전기(?!자재)|주단조|원자로냉각재|"
     r"Steam\s*Generator|Gas\s*Turbine|Steam\s*Turbine|Turbine\s*Generator|Turbine\b", None),
    ("AUX", "보조기기", "BOP / auxiliaries", 5,
     r"열교환기|압력용기|저장탱크|복수기|급수가열기|탈기기|보일러|HRSG|배열회수보일러|"
     r"펌프|밸브|배관|탱크|저장조|팬\b|블로워|집진|탈황|탈질|스크러버|"
     # KC코트렐 수출 계약명의 FGD(탈황)·ESP(전기집진) 약어.
     r"FGD\b|ESP\b|Desulfuri|Precipitator|Boiler\b|Tank\b|Vessel\b|삼중수소제거|"
     # 배전반·차단기·개폐기 같은 전기설비는 발전소에선 보조기기(BOP)다.
     r"배전반|차단기|개폐기|개폐소|수배전|변압기|전동기|케이블|GIS\s*\(", None),
    ("SUPPLY", "기자재 공급", "Equipment supply", 0,
     r"기자재|자재|부품|소재|공급\s*계약|납품|제작\s*공급|제작\s*(?:및\s*)?설치|구매|공급", None),
]
