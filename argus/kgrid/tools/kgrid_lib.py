...
PRODUCT_SLOTS = [
    ...
    # 회색 계열은 **분류가 약한 갈래**다(색으로도 그 뜻을 보인다).
    # `tr_unknown` 은 원문이 그냥 `변압기` 라고만 적어 전압 계급을 알 수 없는 경우다 —
    # HD현대일렉트릭 2026 반기 II절에는 `초고압`·`345kV` 가 한 번도 안 나온다(실측).
    # 초고압인지 배전용인지 **추정하지 않고** 이 갈래에 둔다(COMMON §0-1: 모르면 빈칸).
    ("tr_unknown", 9, "변압기(전압 계급 미상)", "#8a93a3"),
