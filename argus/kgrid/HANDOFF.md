...
- 모집단 **34종목** — 업종 9 · 지정 15 · **탐색 7**(본문 인용으로 승격) · 지주 3(집계 제외).
  어휘로는 걸렸지만 **본문 근거로 뺀 5사**는 `universe.json` 의 `demoted` 에 이유와 함께 남았다.
- 정기보고서 **41사 × 6분기, 실패 0**(빠진 회사 캐시도 보존 — 되살릴 때 재수집이 필요 없다).
- 배수 중앙값 **1.11년**, **장납기 9사 : 회전 8사** — 스펙의 이분 가설이 전수로 확인됐다.
- 외화로 잔고를 공시하는 회사 1사(일진전기 `천USD`) — 환산하지 않고 배수를 비웠다.
- 계약 공시 **자사 176건**(지주·모회사의 자회사 재공시 76건은 갈라 놓았다) · 발주처·지역 집계.
- 페이지 37장(허브·단선도·커버리지 + 회사 34). 테스트 **105건 통과**.
  모집단에서 빠진 회사의 쪽은 `kgrid_page.py --all` 이 지운다.
...
- 원장을 **2021Q4~2026Q2 19분기**로 확장했다(이전 6~8분기). 재현: `python3 tools/kgrid_reports.py --collect --quarter 2026Q2 --n 19`
  → `--build` → `tools/kgrid_page.py --all`. (조선은 `kship_yards.py --collect --quarter <분기>` 를 분기마다.)
- 스튜디오 Codex(gpt-6-astra/ultra)가 만든 **Y+2 추정**(분기 T+1~T+10 · 연간 FY2026~FY2028)이 있으면
  `tools/assets/forecast_panel.json.gz` + `tools/kgrid_forecast_section.py` 로 회사 페이지에 실린다.
